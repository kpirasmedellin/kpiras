...
export default function Menu() {
  ...
  const fetchProductsAndCategories = async () => {
    ...
    try {
      ...
      setCategories([
        "Todas",
        ...categoryData.map((cat: { nombre: string }) => cat.nombre),
      ]);
    } catch (error) {
  ...
  const handleProductAdded = async () => {
    try {
      // Refresca toda la lista de productos después de agregar
      await fetchProductsAndCategories();

      // Cierra el modal y muestra una única alerta
      setIsModalOpen(false);
      InputAlert("Producto añadido exitosamente", "success");
    } catch (error) {
      InputAlert("Error refrescando la lista de productos", "error");
    }
  };
